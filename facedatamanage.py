...
class FaceDataManageUI(QWidget):
    logQueue = multiprocessing.Queue()  # 日志队列
    receiveLogSignal = pyqtSignal(str)  # 日志信号

    def __init__(self):
        super(FaceDataManageUI, self).__init__()
        loadUi('./ui/face_datamanage.ui', self)
    ...
    def deleteUser(self):
        ...
        ret = FaceDataManageUI.callDialog(QMessageBox.Warning, text, informativeText, QMessageBox.Yes | QMessageBox.No,
                                      QMessageBox.No)

        if ret == QMessageBox.Yes:
            stu_id = self.UsernumlineEdit.text()
            cn_name=self.UsernamelineEdit.text()
            conn = sqlite3.connect(self.database)
            cursor = conn.cursor()
            ...
            try:
                ...
                cursor.close()
                conn.commit()
                if os.path.exists('{}/{}.jpg'.format(self.datasets, cn_name)):
                    try:
                        os.remove('{}/{}.jpg'.format(self.datasets, cn_name))
                    except Exception as e:
                        logging.error('系统无法删除删除{}/{}.jpg'.format(self.datasets, cn_name))
                        self.logQueue.put('Error：删除人脸数据失败，请手动删除{}/{}.jpg'.format(self.datasets, cn_name))

                text = '你已成功删除学号为 <font color=blue>{}</font> 的用户记录。'.format(stu_id)
                #informativeText = '<b>请在右侧菜单重新训练人脸数据。</b>'
               # FaceDataManageUI.callDialog(QMessageBox.Information, text, informativeText, QMessageBox.Ok)
    ...
    #         ret = FaceDataManageUI.callDialog(QMessageBox.Question, text, informativeText,
